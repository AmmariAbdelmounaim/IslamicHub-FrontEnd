...
} from "../../../../../redux/features/aboutUsApiSlice";
...
function AboutUsSection() {
  ...
  const initialValues: FormValues = {
    aboutUss: (userInfo?.centerDTO.homePageDTO.aboutUsDTOList as AboutUs[]) ?? [
      {
        title: "",
        paragraph: "",
      },
        ...
        <div className="flex flex-col gap-[40px]">
          <div className="flex flex-col gap-[24px]">
            ...
            <p className="font-poppins text-primary-orange-dark-active text-[18px] capitalize">
              this is an about section for you to provide information about your
              islamic center, your mission, and maybe vision. every information
              is valuable. and can help grow the community of the center.
            </p>
          </div>
          ...
          <Formik
            ...
            onSubmit={async (values, { setSubmitting }) => {
              try {
                ...
                for (const aboutUs of values.aboutUss) {
                  const aboutUsResponse = await editAboutUs({
                    id: aboutUs.id,
                    token: userInfo?.token,
                    title: aboutUs.title,
                    paragraph: aboutUs.paragraph,
                  });
                  if ("data" in aboutUsResponse) {
                    dispatch(updateAboutUs({ ...aboutUsResponse.data }));
                  }
                  // Optional: log the response for each event if you need
                  console.log("aboutUs response: ", { aboutUsResponse });
                }
                  ...
                  <FieldArray
                    ...
                    render={(arrayHelpers) => (
                      <div className="flex flex-col gap-[40px] items-center">
                        {(userInfo?.centerDTO.homePageDTO.aboutUsDTOList
                          .length as number) > 0 &&
                            ...
                            (aboutUS: AboutUs, index) => (
                              ...
                              >
                                ...
                                <CustomField
                                  ...
                                  name={`aboutUss[${index}].title`}
                                />
                                <CustomField
                                  label="paragraph content"
                                  placeholder="Provide a detailded paragraph"
                                  textarea
                                  name={`aboutUss[${index}].paragraph`}
                                />
                                <div className="flex items-center justify-center">
                                  <FillButton
                                    additionalStyle="pt-[12px] px-[40px] gap-[10px] w-[280px]"
                                    type="button"
                                    onClick={async () => {
                                      try {
                                        const aboutUsRes = await deleteAboutUs({
                                          id: aboutUS.id,
                                          token: userInfo?.token,
                                        }).unwrap();
                                        console.log(
                                        ...
                                        dispatch(
                                          removeAboutUs({ id: aboutUS.id })
                                        );
                                        arrayHelpers.remove(index);
                                        console.log("values: ", values);
                                        toast.success(
                                          `Paragraph ${index + 1} deleted !`,
                                          {
                                            position: "bottom-right",
                                            autoClose: 5000,
                                            hideProgressBar: false,
                        ...
                        <FillButton
                          ...
                          type="button"
                          onClick={async () => {
                            const aboutUsRes = await createAboutUs({
                              id: 0,
                              title: "",
                              paragraph: "",
                            ...
                            console.log("about us response: ", aboutUsRes);
                            dispatch(setAboutUs({ ...aboutUsRes }));
                            arrayHelpers.push(aboutUsRes);
                            console.log("values: ", values);
